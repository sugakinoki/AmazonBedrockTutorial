...
function explainError(response, raw) {
  ...
  if (response.status === 403 || type === 'AccessDeniedException') {
    return (
      'Bedrock に拒否されました。よくある原因は次の3つです。\n' +
      '1. API キーの期限切れ。短期キーは最長12時間で失効します。' +
      'コンソールで再発行して .env に貼り替えてください。\n' +
      '2. モデルアクセスが未有効。Bedrock コンソールの「モデルアクセス」で、' +
      '使いたいモデルを有効化してください。\n' +
      '3. そのモデル固有の追加条件を満たしていない。' +
      '（例: OpenAI GPT-5.6 系は既定プロジェクトへの権限が必要、' +
      'Claude Fable 5 はデータ保持設定のオプトインが必要）' +
      detail
    );
  }
